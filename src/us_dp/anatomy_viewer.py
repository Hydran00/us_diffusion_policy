...
from .anatomy import estimate_surface_frame
from .anatomy_assets import sha256
...
def view_anatomy(directory, *, screenshot=None, close_after_capture=False):
    ...
    if not viewer.create_window(
        "US-DP | Phantom PCA: 1=RED 2=GREEN | Liver center=YELLOW | P:skin L:liver S:PNG", 1280, 900
    ):
        raise RuntimeError(
            "Open3D could not open a window. Run from a graphical terminal or use anatomy --no-view."
        )
    try:
        for geometry in [wire, liver_wire, marker, lines, *arrows]:
